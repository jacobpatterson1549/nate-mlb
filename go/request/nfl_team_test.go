...
func TestNflTeamRequestScoreCategory(t *testing.T) {
	...
	for i, test := range RequestScoreCategoryTests {
		...
		nflTeamR := nflTeamRequester{requester: r}
		got, err := nflTeamR.RequestScoreCategory(db.PlayerTypeNflTeam, db.PlayerTypeInfo{}, 2019, test.friends, test.players)
		switch {
		case test.wantErr:
			if err == nil {
...
func TestNflTeamPlayerSearchResults(t *testing.T) {
	...
	for i, test := range playerSearchResultsTests {
		...
		r := newMockHTTPRequester(jsonFunc)
		nflTeamR := nflTeamRequester{requester: r}
		got, err := nflTeamR.Search(db.PlayerTypeMlbTeam, 2019, test.playerNamePrefix, true)
		switch {
		case test.wantErr:
			if err == nil {
				t.Errorf("Test %v: wanted error but did not get one", i)
